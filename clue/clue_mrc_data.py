...
class MRCDataset(Dataset):
    ...
    def __init__(self, split='train', verbose=True, args=None, max_length: int = 512, mode='train'):
        ...
        for idx, line in enumerate(lines):
            ...
            if args.task_name == 'c3': # context question answer
                ...
                if 'test' not in split:
                    ...
                    label = '0'
                context_entrys = line['context_entrys']
                question_entrys = line['question_entrys']
                self.lines.append({'text_idx':str(idx), 'label': label, 'context':context, 'question':question, 'answers':answers, 'entrys':context_entrys+question_entrys})
                self.id2datum[str(idx)] = {'label':label, 'context':context, 'question':question, 'answer':answers, 'entrys':context_entrys+question_entrys}
    ...
    def __getitem__(self, idx):
        ...
        text_idx,label,context,question,answers,entrys = line['text_idx'],line['label'],line['context'],line['question'],line['answers'],line['entrys']
        # entrys
        ...
        for answer in answers:
            answer_tokens = self.tokenizer.tokenize(answer.lower())
            context_tokens, answer_tokens, question_tokens = self.truncate_seq_tuple(context_tokens[:], answer_tokens[:], question_tokens[:], self.args.max_text_length-4)
            src_a = ['[CLS]'] + context_tokens + ['[SEP]']
            seg_a = [0] * len(src_a)
            src_b = question_tokens + ['[SEP]'] + answer_tokens + ['[SEP]']
            seg_b = [1] * len(src_b)
    ...
    def get_labels(self, ):
        if self.args.task_name == 'c3':
            return ['0', '1', '2', '3']
        else:
            raise ValueError('INVALID TASK')


    def truncate_seq_tuple(self, tokens_a, tokens_b, tokens_c, max_length):
