...
class BertDictTC(BertPreTrainedModel):
    ...
    def train_step(self, batch):
        ...
        # batch_dict_embeds = []
        # for i in range(B):
        #     batch_dict.append(desc_ids[i].to(device))
        #     batch_dict_embeds.append(desc_ids[i].to(device).mean(0))
        # batch_dict = []
        ...
        if self.fuse == 'mean':
            ...
            # cross attention
            # max_length = max(entry.size(0) for entry in batch_dict)
            # dict_logits = torch.zeros((B, max_length, self.config.hidden_size), dtype=torch.float32, device=device)
            # dict_mask = torch.zeros((B, max_length), dtype=torch.long, device=device)
            # for ii, entry_lg in enumerate(batch_dict):
    ...
    def test_step(self, batch, **kwargs):
        ...
        if self.fuse == 'mean':
            # mean
            # fused_logits = torch.stack(batch_dict_embeds)
            fused_logit = entry_ids.sum(1)
            pooled_output = self.concat_pooler(torch.cat((bert_logits[:,0], fused_logits), dim=1))
            logits = self.classifier(self.dropout(pooled_output))
...
class OriBert(BertForSequenceClassification):
    def __init__(self, config):
        super().__init__(config)
        # self.num_labels = 15
        # classifier_dropout = (
    ...
    def train_step(self, batch):
        ...
        input_desc_ids = batch['input_desc_ids'].to(device)
        input_desc_token_type_ids = batch['input_desc_token_type_ids'].to(device)
        input_desc_mask = batch['input_desc_mask'].to(device)
        
        output = self(
            input_ids=input_desc_ids,
            attention_mask=input_desc_mask,
            token_type_ids=input_desc_token_type_ids,
            labels=label
        )
