...
def corrupt_ft_dict(word, sentence, radical2idx, rid, tokenizer):
    
    print(word, sentence)
    
    word_tokens = tokenizer.tokenize(word)
    n_word = len(word_tokens)
    word_radical = [radical2idx.get(x, rid) for x in word_tokens]
    
    
    source_sentence = re.sub(word, '[MASK]'*n_word, sentence)
    source_text = '[MASK]'*n_word + '[SEP]' + source_sentence
    target_text = word + '[SEP]' + sentence
    
    target_radical = [radical2idx.get(x, rid) for x in tokenizer(sentence)]
    source_radical = [radical2idx.get(x, rid) for x in tokenizer(source_sentence)]
    

    
    source_radical = [0]*n_word + [rid] + source_radical
    target_radical = word_radical + [rid] + target_radical
    
    
    # if '[例子]：' in sentence:
    #     desc, example = sentence.split('[例子]：', 1)
    #     source_example = re.sub(word, '[MASK]'*n_word, example)
    #     desc_radical = [radical2idx.get(x, rid) for x in tokenizer.tokenize(desc)]
    #     example_radical = [radical2idx.get(x, rid) for x in tokenizer.tokenize(example)]
    #     source_example_radical = [radical2idx.get(x, rid) for x in tokenizer.tokenize(source_example)]
    #     source_text = '[MASK]'*n_word + '[SEP]' + desc + '例子:' +source_example
    #     target_text = word + '[SEP]' + desc + example
    #     source_radical = [0]*n_word + [rid] + desc_radical + [0]*3 + source_example_radical
    #     target_radical = word_radical + [rid] + desc_radical + [0]*3 + example_radical
        
    # else:
    #     desc, example = sentence, ''
    #     source_example = ''
    #     desc_radical = [radical2idx.get(x, rid) for x in tokenizer.tokenize(desc)]
    #     source_text = '[MASK]'*n_word + '[SEP]' + desc 
    #     target_text = word + '[SEP]' + desc
    #     source_radical = [0]*n_word + [rid] + desc_radical 
    #     target_radical = word_radical + [rid] + desc_radical
        
    
    print(source_text, target_text, source_radical, target_radical)
    return source_text, target_text, source_radical, target_radical

def corrupt_dict(word, sentence, tokenizer, max_length):
    ...
    mask_sentence_tokens = sentence_tokens[:]
    for i in range(len(sentence_tokens)-n_word+1):
        if sentence_tokens[i:i+n_word] == word_tokens:
            mask_sentence_tokens[i:i+n_word] = ['[MASK]']*n_word
    
    n_sent = min(len(sentence_tokens), max_length-3-n_word)
    
    source_tokens = ['[CLS]'] + ['[MASK]']*n_word + ['[SEP]'] + mask_sentence_tokens[:n_sent] + ['[SEP]']
    target_tokens = ['[CLS]'] + word_tokens + ['[SEP]'] + sentence_tokens[:n_sent] + ['[SEP]']
    token_types = [0] * (n_word+1) + [1] * (len(source_tokens)-n_word-1)
    ...
    return source_ids, target_ids, token_types



def corrupt_radical_dict(word, sentence, radical2idx, rid, tokenizer, max_length):
    ...
    source_radical = [radical2idx.get(x, rid) for x in source_tokens]
    target_radical = [radical2idx.get(x, rid) for x in target_tokens]
    for i in range(len(target_tokens)):
        if target_tokens[i] == '[MASK]':
            target_radical[i] = 0
    
    source_ids = tokenizer.convert_tokens_to_ids(source_tokens)
    target_ids = tokenizer.convert_tokens_to_ids(target_tokens)
    
    return source_ids, target_ids, source_radical, target_radical, token_types
