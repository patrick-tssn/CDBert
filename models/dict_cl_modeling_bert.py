...
from transformers.models.bert.modeling_bert import(
    BertEncoder, BertModel, BertForMaskedLM, BertConfig, BertForSequenceClassification, BertForPreTraining
)
...
class BertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if glyph_embeds is not None:
            glyph = self.glyph_embeddings(glyph_embeds)
            inputs_embeds = torch.cat((inputs_embeds, glyph), dim=1)
...
class BertForDictPretraining(BertForPreTraining):
    ...
    def forward(
        ...
        if labels is not None and syno_input_ids is not None:
            cl_loss = self.cl_loss(sequence_output[:, 0], syno_sequence_output[:, 0], anto_sequence_output[:, 0])
        ...
        if not return_dict:
            ...
            return ((masked_lm_loss,cl_loss) + output) if total_loss is not None else output
