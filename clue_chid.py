...
from clue.clue_chid_data import get_loader
from clue.clue_chid_model import BertDictMRC
    ...
    def predict(self, loader, dump_path=None):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(loader):
                ...
                for tid, eid, logit in zip(text_id, example_id, logits):
                    if eid not in raw_results:
                        raw_results[eid] = [(tid, logit)]
                    else:
                        raw_results[eid].append((tid, logit))
                
                if self.verbose:
                    pbar.update(1)
            for eid, elem in raw_results.items():
                ...
                logits_matrix = np.transpose(logits_matrix) # num_choices, B
                tmp = []
                for i, row in enumerate(logits_matrix): # 0 -> num_choices
                    for j, col in enumerate(row): # 0 -> B, logit
                        tmp.append((i, j, col)) # choice_index, batch_index, logit
                else:
                ...
                for ans, idx in results:
                    textid2ans[index_2_idiom[idx]] = ans
