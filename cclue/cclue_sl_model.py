...
class BertDictSL(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    def train_step(self, batch):
        ...
        if self.fuse == 'mean':
            ...
            # debug
            logits = self.bert_classifier(self.dropout(bert_logits))
            # logits = self.classifier(self.dropout(pooled_output))
        elif self.fuse == 'lse':
        ...
        if label is not None:
            if self.num_labels == 1:
                ...
                loss_fct = CrossEntropyLoss() # B*L, num_labels 
