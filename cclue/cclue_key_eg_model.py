...
from models.dict_modeling_bert import BertClipModel, BertForDictPretraining, DictRetriever
...
class BertDictMRC(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = BertModel(config)
        # self.dict_bert = BertClipModel(config)
        self.dict_bert = BertForDictPretraining(config)
        self.retriever = DictRetriever(config, 1)
        ...
        self.bert_classifier = nn.Linear(config.hidden_size, 1)
    ...
    def train_step(self, batch):
        ...
        flat_input_types = input_types.view(-1, input_types.size(-1))
        
        bert_output = self.bert(
            input_ids=flat_input_ids,
            attention_mask=flat_input_mask,
            token_type_ids=flat_input_types,
        ...
        #                             )[0]
        # fused_logits = self.dict_block(bert_logits[:, 0].contiguous().view(-1, num_choices, self.config.hidden_size),
        ...
        # self.dict_bert.dict_retriever
        # fused_logits = self.dict_bert.dict_retriever(
        #     bert_logits[:, 0].contiguous().view(-1, num_choices, self.config.hidden_size),
        #     encoder_hidden_states=entry_ids,
        #     encoder_attention_mask=extended_attention_mask,
        #     self_attn=False,
        # )[0]
        fused_logits = self.retriever(
            bert_logits[:, 0].contiguous().view(-1, num_choices, self.config.hidden_size),
            encoder_hidden_states=entry_ids,
            encoder_attention_mask=extended_attention_mask,
            self_attn=False,
        ...
        # # debug
        logits = self.bert_classifier(self.dropout(self.pooler(bert_logits)))
        
        # fused_pooled_logits = self.concat_pooler(torch.cat((bert_logits[:,0], fused_logits.view(-1, self.config.hidden_size)), dim=-1))
        # logits = self.classifier(self.dropout(fused_pooled_logits))
        # # logits = self.bert_classifier(self.dropout(fused_pooled_logits))
