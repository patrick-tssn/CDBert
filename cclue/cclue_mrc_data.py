...
class MRCDataset(Dataset):
    ...
    def __init__(self, split='train', verbose=True, args=None, max_length: int = 512, mode='train'):
        ...
        for idx, line in enumerate(lines):
            ...
            if args.task_name == 'mrc': # context question answer
                answers = line['choices']
                context = line['context']
                question = line['question']
                label = line['label']
                entrys = line['entrys']
                self.lines.append({'text_idx':str(idx), 'label': label, 'context':context, 'question':question, 'answers':answers, 'entrys':entrys})
                self.id2datum[str(idx)] = {'label':label, 'context':context, 'question':question, 'answer':answers, 'entrys':entrys}
    ...
    def __getitem__(self, idx):
        ...
        for answer in answers:
            ...
            context_tokens, question_tokens, answer_tokens = self.truncate_seq_tuple(context_tokens[:], question_tokens[:], answer_tokens[:], self.args.max_text_length-4)
            src_a = ['[CLS]'] + question_tokens + ['[SEP]'] + answer_tokens + ['[SEP]']
